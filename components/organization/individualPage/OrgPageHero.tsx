...
export default function OrgPageHero({ org }: { org: Organization }) {
  const heroMeta: string[] = [];

  heroMeta.push(`${org.package_count || 0} Dataset${org.package_count === 1 ? "" : "s"}`);

  if (org.created) {
    heroMeta.push(`Created ${getTimeAgo(org.created)}`);
  }
